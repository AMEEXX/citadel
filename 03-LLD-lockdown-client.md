...
|---|---|
| Win32 Secure Desktop Isolation | Dedicated isolated Windows Desktop plane created via `CreateDesktopW`. Kiosk browser process is spawned directly on the background plane with `--disable-gpu`, `--disable-gpu-compositing`, and software rasterization flags. Process health and window initialization are verified before physical display output is switched via `SwitchDesktop`. If the browser fails to start or server is unreachable, the lockdown aborts safely without affecting the candidate's normal desktop. |
